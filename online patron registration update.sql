...
Query used in conjunction with update patron.py
Identifies online patron registrations and identifies the ptype they should be assigned
based upon city/town entered in the registration form
*/
...
--treat all villages of Newton as Newton
...
--Limit to online patron ptype, which is assigned to records from the online registration form
WHERE
p.ptype_code = '207'
--Limit to only those towns that fall within our network
AND (
LOWER(a.city) ~ 'acton'
OR LOWER(a.city) ~ 'arlington'
OR LOWER(a.city) ~ 'ashland'
OR LOWER(a.city) ~ 'auburndale'
--include Bedford, exclude New Bedford
...
--include Concord, but not Concord New Hampshire
...
--inlclude Lincoln but not Lincoln Nebraska
